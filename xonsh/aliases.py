...
def source_foreign(args, stdin=None, stdout=None, stderr=None):
    ...
    if ns.prevcmd is not None:
        ...
        ns.prevcmd = f"""{ns.sourcer} "{'" "'.join(ns.files_or_code)}\""""
    else:
    ...
    for k, v in fsaliases.items():
        if k in baliases and v == baliases[k]:
            ...
            baliases[k] = v
        elif not ns.suppress_skip_message:
            msg = (
...
def xexec(args, stdin=None):
    ...
    if len(args) == 0:
        ...
        try:
            ...
            return None, f"xonsh: exec: file not found: {e.args[1]}: {args[0]}\n", 1
